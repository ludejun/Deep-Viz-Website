...
import { MapScatter } from '../../components/src';
import Basic from '../Layout/WDBasic';

export default class MapScatterShow extends Basic {
  onTooltipFormat(params) {
    const tips = `<div class='tooltip-sample'>
      ${params.seriesName}<br>${params.name} : ${params.value[2]}</div>
    `;
    return tips;
  }
  onScatterClick() {
  ...
  render() {
    ...
    const data4 = [{ name: '海门', value: 9 }, { name: '鄂尔多斯', value: 12 }, { name: '舟山', value: 12 }];
    const data5 = [{ name: '齐齐哈尔', value: 14 }, { name: '盐城', value: 15 }];

    const mapScatterSource = [
      {
    ...
    const dataCfgSource = [
      {
        parameter: 'options',
        description: '必需，组件配置选项，详见下面Table',
        type: 'Array',
        defaultValue: 'null',
      },
      {
    ...
    const tableConfig = [
      { title: 'MapScatter', subtitle: '', dataSource: mapScatterSource },
    ...
    return (
      <div>
        <p className="container-header">MapScatter 散点图地图</p>
        <p>基础散点图地图，可用于在地图上绘制不同颜色、不同大小的pointer、scatter，
          每组数据可以拥有不同样式的scatter，每组数据可以通过legend点击显示或消失。
          每个pointer的大小根据数据的最大值最小值自动算出。
        </p>
        ...
        <Card>
          ...
          <p className="sample-desc">scatter、区域颜色可调，scatter形状颜色可改变</p>
          <div className="code-container">
            <Highlight className="JavaScript">
              {`import { MapScatter } from 'deep-viz';
...
, mountNode);
        ...
        <Card>
          ...
          <p className="sample-desc">有主标题，副标题，scatter、区域颜色可调，scatter大小随value大小不同，scatter形状颜色可改变</p>
          <div className="code-container">
            <Highlight className="JavaScript">
              {`import { MapScatter } from 'deep-viz';
