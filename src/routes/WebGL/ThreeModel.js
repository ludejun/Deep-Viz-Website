  ...
  render() {
    const modelSource = [
      ...
      },
      {
        parameter: "modelPath",
        description:
          "非必需，自定义模型文件路径，一般为obj文件，modelPath没有会取modelType，modelPath优先",
        type: "String",
        defaultValue: "null",
      },
      {
      ...
      },
      {
        ...
        description:
          "非必需，点光源位置，为length=3的数组，分别对应x、y、z位置",
        type: "Array",
      ...
      },
      {
        parameter: "cameraZ",
        description:
          "非必需，=camera.position.z，代表摄像头和模型的距离，越小模型越大",
        type: "Number",
        defaultValue: "250",
      },
      {
        parameter: "cameraY",
        description:
          "非必需，=camera.position.y，代表摄像头和模型在Y轴上位置，改变此值会上下移动模型",
        type: "Number",
        defaultValue: "-80",
      },
      {
    ...
    return (
      <div>
        ...
        <p>
          3D的人物模型图，内置2款男女模型，支持自定义添加模型。可用于用户画像等。更多模型可以参考
          <a
            href="https://github.com/mrdoob/three.js/tree/dev/examples/models/obj"
            target="_blank"
          >
            官方github示例
          </a>
        ...
        <Card>
          ...
          <p className="sample-desc">
            默认为男性模型，可以对比两个示例代码的差别，
            给右边模型减小和camera的距离、反向旋转、camera位置随鼠标在屏幕上的位置而变化
          </p>
...
ReactDOM.render(
  <div style={{ overflow: 'hidden' }}>
    <div style={{ float: 'left' }}>
      <ThreeModel modelPath='/model/male02.obj' height="400" width="300" />
    </div>
    <div style={{ float: 'right' }}>
      <ThreeModel
        height="400"
        width="300"
        modelPath='/model/female02.obj'
        ambientLightColor={'#ca7a76'}
        cameraZ={220}
        rotatateY={-0.05}
        isOnMouseMove
...
, mountNode);
        ...
        <Card>
          <ThreeModel
            modelPath={`${
              window.location.href.indexOf("github.io") > -1 ? "/deepviz" : ""
            }/model/standard-male-figure.obj`}
            cameraZ={30}
            cameraY={-10}
          />
          <hr />
          ...
          <p className="sample-desc">
            引入一个非默认模型方法，模型较小，适当减小cameraZ；当模型引入而看不见或尺寸不合适，需适当调整cameraY、cameraZ
          </p>
          <div className="code-container">
            <Highlight className="JavaScript">
              {`import { ThreeModel } from 'deep-viz';
